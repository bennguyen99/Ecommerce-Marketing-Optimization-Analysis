...
WITH sales_merge AS
	...
	FROM online_sales_fmt AS s
	INNER JOIN discount_coupon AS d
	ON s."Product_Category" = d."Product_Category"
	AND s."Month" = d."Month"
	GROUP BY d."Coupon_Code"
	)

SELECT "Coupon_Code",
	ROUND(
	("coupon_used_count"::numeric/ "transaction_count"::numeric)
	, 2) AS "redemption_rate"
FROM sales_merge
ORDER BY redemption_rate DESC;

-- 6. Impact of each coupon code on profit margin
...
-- 7. Identify buyers who did not use coupons for remarketing

WITH sales_merged AS
	(SELECT "Transaction_ID", o."CustomerID", "Coupon_Status", 
	COUNT ("Transaction_ID") AS transaction_count,
	COUNT ("Coupon_Status") AS status_count
	FROM online_sales_fmt AS o
	INNER JOIN customers AS c
	ON o."CustomerID" = c."CustomerID"
	INNER JOIN discount_coupon AS d
	ON o."Product_Category" = d."Product_Category"
	AND o."Month" = d."Month"
	GROUP BY "Transaction_ID", o."CustomerID", "Coupon_Status"
	)

SELECT DISTINCT ("CustomerID")
FROM sales_merged
WHERE "Coupon_Status" NOT IN ('Used')
AND transaction_count = status_count

/* The remarketing list contains a total of 1440 customers */